...
:: --- Maven -------------------------------------------------------------------
:: First try PATH, then JAVA_HOME sibling, then common install locations
...
if not defined MVN_CMD (
    call :find_file_recursive "%USERPROFILE%\.m2"      "mvn.cmd" MVN_CMD
)
if not defined MVN_CMD (
    ...
    call :find_file_recursive "%LOCALAPPDATA%\Microsoft\WinGet\Packages" "mvn.cmd" MVN_CMD
    if not defined MVN_CMD (
        echo [!!]  Maven installed but could not locate mvn.cmd.
        echo       Re-open this terminal so mvn is on PATH, then re-run build.bat.
        exit /b 0
    )
)
:: Ensure the folder containing mvn.cmd is on PATH for this session
for %%f in ("!MVN_CMD!") do set "MVN_BIN=%%~dpf"
set "MVN_BIN=!MVN_BIN:~0,-1!"
echo "!PATH!" | findstr /i /c:"!MVN_BIN!" >NUL 2>&1
if errorlevel 1 set "PATH=!MVN_BIN!;!PATH!"
for /f "tokens=3" %%v in ('"!MVN_CMD!" --version 2^>^&1 ^| findstr /i "Apache Maven"') do echo [OK]  Maven %%v

:: --- jpackage (bundled with JDK 14+) -----------------------------------------
:: Check PATH first, then JAVA_HOME\bin, then common JDK install locations
...
::   Sets result_var to the full path if found on PATH, else leaves it unset.
:: ============================================================
:find_on_path
set "%~2="
...
::   Searches root recursively for filename; sets result_var to first match.
::   Skips silently if root does not exist.
:: ============================================================
:find_file_recursive
