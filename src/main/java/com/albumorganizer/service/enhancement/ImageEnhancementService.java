...
public class ImageEnhancementService {
    ...
    public static final List<NamedPrompt> DEFAULT_PROMPTS = List.of(
        new NamedPrompt(-1, "Adult Content Filter",
        ...
        new NamedPrompt(-2, "Upscale & Sharpen",
        ...
        new NamedPrompt(-4, "Restore Old Photo",
        ...
        new NamedPrompt(-5, "Enhance Portrait",
        ...
        new NamedPrompt(-6, "Increase Clarity",
        ...
        new NamedPrompt(-7, "Fix Low Light",
        ...
        new NamedPrompt(-8, "Vivid Colors",
        ...
        new NamedPrompt(-9, "Make Black & White",
    ...
    /** Returns true if the given prompt is a built-in default (id < 0). */
    public static boolean isDefaultPrompt(NamedPrompt p) {
        return p.isDefault();
    }

    /**
     * Returns the saved prompts, prepending any default prompts whose IDs are not
     * already present. Missing defaults are taken from the constants, not from storage.
     */
    public static List<NamedPrompt> seedPrompts(List<NamedPrompt> savedPrompts) {
        ...
        for (NamedPrompt p : result) existingIds.add(p.id());
        int insertAt = 0;
    ...
    public ImageEnhancementService(SettingsRepository settingsRepository) {
    ...
    public void reloadProviders() {
        providers.clear();
        EnhancementConfig cfg = settingsRepository.load().toEnhancementConfig(
            new com.albumorganizer.repository.UsageRepository().load());
     ...
     * Pattern: {base}_{provider}-{epoch}.jpg — always JPEG.
     */
    public static Path resolveOutputPath(Path inputPath, String providerName) {
        ...
        String candidate = String.format("%s_%s-%d.jpg", base, providerSlug, epoch);
